						...
						<table class="table table-hover mb-0">
							...
							<tbody>
								<c:forEach var="account" items="${listAccounts}">
									...
									<td><a href='<c:url value='admin-edit-freelancer?id=${account.id}'/>'><i
		...
		<c:if test="${totalPage>1}">
			<ul class="pagination">
				...
				<c:if test="${previousPage!=-1}">
					<li class="page-item"><a class="page-link"
						href="<c:url value="admin-manager-freelancer?page=${pageIndex-1}"></c:url>">&lt;</a></li>
				</c:if>
				<c:forEach var="i" begin="${startPageIndex}"
					end="${endPageIndex}">
					<c:if test="${i==pageIndex }">
						<li class="page-item active"><a class="page-link"
							href="<c:url value="admin-manager-freelancer?page=${i}"></c:url>"><c:out
									value="${i}"></c:out></a></li>
					</c:if>
					<c:if test="${i!=pageIndex }">
						<li class="page-item"><a class="page-link"
				...
				<c:if test="${pageIndex !=totalPage }">
					<li class="page-item"><a class="page-link"
						href="<c:url value="admin-manager-freelancer?page=${totalPage}"></c:url>">Trang
							cuôi</a></li>
				</c:if>
			</ul>
		</c:if>
